...
export default function Perast() {
  ...
  return (
    <ContentPage title={t("perast.title")} subtitle={t("perast.subtitle")} description={t("perast.seoDesc")} image="/img/perast-village.webp">
      <img src="/img/our-lady-rocks.webp" alt={t('imageAlts.perastWaterfront')} loading="lazy" />
      ...
      <img src="/img/pexels-7510133.jpg" alt={t('imageAlts.bayOfKotorNearPerast')} loading="lazy" />

      <h2>{t('perastBody.historicTitle')}</h2>
      <p>{t('perastBody.historicText')}</p>

      <h3>{t('perastBody.monasteryTitle')}</h3>
      <p>{t('perastBody.monasteryText')} {t('perastBody.routeLinkBefore')} <a href={localePath('/montenegro')}>{t('perastBody.routeLinkText')}</a>{t('perastBody.routeLinkAfter')}</p>
    </ContentPage>
  );
}
